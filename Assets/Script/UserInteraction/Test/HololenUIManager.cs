...
public class HololenUIManager : MonoBehaviour
{
    ...
    public void LoadReservatedDataUpdate()
    {
        ...
        foreach (string key in keys)
        {
            if (timers[key] >= 3600)
            {
                ...
                ReservatedDataText[0].text = (timers[key] / 3600).ToString("F0") + "시간 " +
                                             ((timers[key] % 3600) / 60).ToString("F0") + "분 이내";
                ReservatedDataText[1].text =
                    DateTime.Now.AddSeconds(timers[key])
    ...
    /// </summary>
    public void AddMatchingRequestData(string partnerObjectName) // !!! ����� ���� ����� �κ�
    {
        ...
        MatchingRequestButton[0].OnClicked.AddListener(() =>
        {
            ...
            notificationManager.SendAcceptMessage(partnerObjectName);
            LoadMatchingStartPopupUITextFromDB();
        ...
        {
            RemoveMatchingRequestData(newObject);
            notificationManager.SendDeclineMessage(partnerObjectName);
            SetTime(0);
    ...
    {
        FileLogger.Log("Add Reserved Data", this);
    ...
    public void LoadReceiveRequestDetailTextFromDB()
    {
        ...
        /*// 디테일 부분의 수락 버튼에 파트너 이름을 넣기 위한 코드, 타이머 부분
        dstring partnerName = UserMatchingManager.Instance.partnerUserInfo.PhotonUserName;
        MeetingManager.Instance.timePicker.CachePartnerName(partnerName);*/
    }
    ...
    public void SetAndShowCommonInterestsText(List<string> commonInterests)
    {
        ...
        if(commonInterests.Count > 0)
            commonInterestsPopupText[0].text 
            = DatabaseManager.Instance.getUserData(UserMatchingManager.Instance.GetPartnerUserPinNumber()).name + "님과 같은 관심을 가지고 있네요!";

        // TODO: 공통된 데이터에 따른 처리
        for (int i = 1; i <= commonInterests.Count; i++)
        {
            commonInterestsPopupText[i].text = commonInterests[i-1];
            commonInterestsPopupText[i].gameObject.SetActive(true);
            
            FileLogger.Log($"나와 파트너의 공통 관심사 {i}번: {commonInterests[i-1]}");
        }
    } 
    ...
    public void ShowMeetingUI()
    {
        ...
        distanceToPartner = (int)temp.magnitude;
    }
