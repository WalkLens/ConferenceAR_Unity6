...
{ 
    ...
    /// </summary>
    /// <param name="user1">첫 번째 사용자 게임오브젝트 이름</param>
    /// <param name="user2">두 번째 사용자 게임오브젝트 이름</param>
    /// <returns>형식: "yyMMdd-HH:mm-ssfff_정렬된유저1-유저2"의 문자열</returns>
    public static string GenerateMatchKey(string user1, string user2)
    {
        // "CentralHost" 문자열 제거
        user1 = user1.Replace("CentralHost", "").Trim();
        user2 = user2.Replace("CentralHost", "").Trim();
        
        // 예외 처리: 두 유저 이름이 모두 빈 문자열이 된 경우
        if (string.IsNullOrEmpty(user1) || string.IsNullOrEmpty(user2))
        {
            Debug.LogError("GenerateMatchKey: 유효한 매칭 대상이 없습니다.");
            return string.Empty;
        }

        // 날짜 및 시간 포맷 (yyyy-MM-dd HH:mm:ss)
        ...
        // 매칭 키 조합
        string matchKey = $"{timestamp}={sortedUsers[0]}-{sortedUsers[1]}";
    ...
    {
        FileLogger.Log($"ExtractOtherUser: matchKey = {matchKey}");

        // 매칭 키 구조 확인
        string[] parts = matchKey.Split('=');
        if (parts.Length < 2) 
        {
            FileLogger.Log("ExtractOtherUser: Invalid matchKey format!");
            return string.Empty; // 잘못된 형식이면 반환 X
        }

        // 유저 정보 추출
        string[] users = parts[1].Split('-'); // "유저1-유저2" 부분 분리

        FileLogger.Log($"ExtractOtherUser: users[0] = {users[0]}, users[1] = {users[1]}");
        myName = myName.Replace("CentralHost", "").Trim();
        FileLogger.Log($"ExtractOtherUser: myName = {myName}");

        // 내 이름이 포함된 경우 상대방 반환 (공백 제거 및 대소문자 무시)
        if (users[0].Equals(myName)) return users[1];
        if (users[1].Equals(myName)) return users[0];

        FileLogger.Log("ExtractOtherUser: My name not found in matchKey!");
