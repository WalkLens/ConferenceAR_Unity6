...
{
    public WebViewerSpawner Spawner;
    ...
    public string GetPartnerUserPinNumber()
    {
        ...
        else if (partnerGameObject != null)
        {
            Debug.Log("Using partnerGameObject instead");
            return partnerGameObject.name[..5];
        }
        else
        {
            Debug.LogError("Both partnerUserInfo and partnerGameObject are NULL!");
            return "Null";
        }
    }
    ...
    private void HandleSendViewProfileEvent(EventData photonEvent)
    {
        ...
        if (data != null && data.Length > 0)
        {
            ...
            notificationManager.OpenProfileUI(receivedpin);
        }
        else
        {
    ...
    private void HandleSendURLDataEvent(EventData photonEvent)
    {
        ...
        if (data != null && data.Length > 0)
        {
            ...
            Spawner.targetURL = receivedURL;
            Spawner.SpawnWebView();
        }
        else
        {
    ...
    private void HandleSendMatchInfoEvent(EventData photonEvent)
    {
        ...
        {
            FileLogger.Log(
                $"UserWhoSend: {receivedMatchInfo.UserWhoSend}, UserWhoReceive: {receivedMatchInfo.UserWhoReceive}, MatchRequest: {receivedMatchInfo.MatchRequest}",
                this);
            // TODO: 매칭 요청 처리
            ...
            {
                if (!myGameObject)
                    myGameObject = FindObjectsOfType<GameObject>()
                        .FirstOrDefault(obj => obj.name.Contains(myUserInfo.PhotonUserName));

                partnerUserInfo = null;
                partnerUserInfo = new UserInfo();

                string partnerUserObjectName = receivedMatchInfo.UserWhoSend;
                partnerUserInfo.PhotonUserName = partnerUserObjectName;
                ...
                HololenUIManager.Instance.AddMatchingRequestData(partnerUserObjectName);
            ...
            else if (debugUserInfo.receivedMatchInfo.MatchRequest == "Accept") // 매칭 응답(Yes)을 받음
            {
                ...
                if (!myGameObject)
                    myGameObject = FindObjectsOfType<GameObject>()
                        .FirstOrDefault(obj => obj.name.Contains(myUserInfo.PhotonUserName));
                FindPartnerHololensGameObject(receivedMatchInfo.UserWhoSend);
                ...
                if (partnerGameObject != null)
                    FileLogger.Log(
                        $"파트너 게임 오브젝트 설정[Accept 받음], Target GameObject Name: {partnerGameObject.name}, UserWhoSend: {receivedMatchInfo.UserWhoSend}",
                        this);
                else
                    FileLogger.Log(
                        $"파트너 게임 오브젝트 찾지 못함[Accept 받음], Target GameObject Name: {receivedMatchInfo.UserWhoSend}", this);

                debugUserInfo.ShowReceiveAcceptUI();
                HololenUIManager.Instance.LoadReceiveAcceptPopupTextFromDB();
                HololenUIManager.Instance.LoadMatchingStartPopupUITextFromDB();
    ...
    /// <summary>
    /// 씬에 존재하는 파트너 게임오브젝트(HMD)를 찾고 partnerGameObject에 할당합니다.
    /// </summary>
    /// <param name="partnerUserName">찾고자 하는 파트너 이름</param>
    public void FindPartnerHololensGameObject(string partnerUserName)
    {
    ...
    private void HandleSendMeetingInfoEvent(EventData photonEvent)
    {
        ...
        if (meetingInfo != null)
        {
            ...
            FileLogger.Log("Match Key: " + meetingInfo.MatchKey, this);
            FileLogger.Log("Meeting Time: " + meetingInfo.MeetingDateTime, this);

            string partnerObjectName = MatchingUtils.ExtractOtherUser(meetingInfo.MatchKey, myUserInfo.PhotonUserName);
            FileLogger.Log($"🔍 Expected Partner Object Name: {partnerObjectName}", this);

            if (!myGameObject)
                myGameObject = FindObjectsOfType<GameObject>()
                    .FirstOrDefault(obj => obj.name.Contains(myUserInfo.PhotonUserName));
            /*partnerGameObject = FindObjectsOfType<GameObject>()
                .FirstOrDefault(obj => obj.name.Contains(partnerObjectName));*/
            // partnerGameObject = GameObject.Find(partnerObjectName);
            FindPartnerHololensGameObject(partnerObjectName);

            FileLogger.Log($"Found Partner Name: {partnerGameObject.name}");


            if (partnerGameObject != null)
                FileLogger.Log(
                    $"파트너 게임 오브젝트 설정[Accept 받음], Target GameObject Name: {partnerGameObject.name}",
                    this);
            else
                FileLogger.Log(
                    $"파트너 게임 오브젝트 찾지 못함[Accept 받음], Target GameObject Name: {partnerObjectName}", this);
            ...
            if (MeetingManager.Instance &&
                (PhotonNetwork.NickName.Contains("hololens") || PhotonNetwork.NickName.Contains("Quest")))
            {
    ...
    IEnumerator ShowCommonInterests(string otherUserPinNumber)
    {
        ...
        List<string> otherUsersInterests = new List<string>()
            { otherUsersData.interest_1, otherUsersData.interest_2, otherUsersData.interest_3 };
        ...
        if (MatchingUtils.CheckCommonInterests(otherUsersInterests, myInterests, out List<string> commonInterests))
        {
            ...
            yield return new WaitForSeconds(5f); // 5초간 보여주고 끈다.
